...
@dp.message(Command('help')) # создаем команду help
...
@dp.callback_query(F.data == 'command') # если кнопка command была нажата выводим эти кнопки
...
@dp.callback_query(F.data == 'start') # если кнопка start была нажата выводим эту информацию о ней
async def callback(callback:types.CallbackQuery): 
    await callback.message.answer('Команда (start) используется для\n записи данных пользователя в базу данных.')
    
@dp.callback_query(F.data == 'weight') # если кнопка start была нажата выводим эту информацию о ней
async def callback(callback:types.CallbackQuery): 
    await callback.message.answer('Команда (weight) ипользуется для\n записи веса пользователя, а так-же выводит его прогресс.')

@dp.callback_query(F.data == 'check') # если кнопка start была нажата выводим эту информацию о ней
async def callback(callback:types.CallbackQuery): 
